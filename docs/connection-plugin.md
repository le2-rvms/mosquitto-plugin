...
- 背景：需要记录设备连接与断开事件，形成可查询的连接历史与当前会话状态。
- 目标：记录 `connect`/`disconnect` 两类事件；写入 `client_conn_events` 与 `client_sessions`。
- 非目标：不参与认证与 ACL；不处理消息转发或入队。
